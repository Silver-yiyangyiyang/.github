...
当前统一优先级、依赖和证据门禁见[项目计划](../PROJECT_PLAN.md)（**V2.0：功能冻结，进入演示视频拍摄**）。人机签字与平台上传见[收口总清单](../HUMAN_CLOSEOUT_LEDGER.md)。
...
## 当前收口重点（2026-09-03 功能冻结后）

1. **立即拍摄演示视频**：按 `jbgs-submission`《演示视频脚本》七段成片；与冻结版本一致并脱敏。静态截图不能替代成片。
2. 并行完成 [人工收口总清单](../HUMAN_CLOSEOUT_LEDGER.md)：签字盖章、报名表、平台上传与回执。
3. 微信小程序保持冻结；Clients #121 / Mobile #28 维持 Hold，不新开功能实现。
4. Mobile 训练授权保持 `unverified`；萤石 Token Switch / 智能体条款未核验前，不把影子写成生产干预。
5. 严格区分模拟、历史、设备受控与真实部署证据；未取得平台回执前不得宣称赛事提交完成。
